...
export function classifyOpportunity(item, now = new Date()) {
  ...
  else if (/full[- ]?time|полная занятость|в штат|оформление по тк/iu.test(text)) classification = OpportunityType.FULL_TIME_JOB;
  ...
  else if (includesAny(text, PROJECT)) classification = OpportunityType.ONE_OFF_PROJECT;
