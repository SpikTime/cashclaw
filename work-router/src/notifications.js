...
export function notificationBand(score, thresholds = {}) { const hot = Number(thresholds.hot ?? 80); const good = Number(thresholds.good ?? 67); const maybe = Number(thresholds.maybe ?? 52); return score >= hot ? "HOT" : score >= good ? "GOOD" : score >= maybe ? "MAYBE" : "SILENT"; }
function details(item) { const sources = item.foundInSources > 1 ? `\nНайдено в ${item.foundInSources} источниках` : ""; const budget = item.budget ? `\nБюджет: ${item.budget.raw || `${item.budget.min || "?"}-${item.budget.max || "?"} ${item.budget.currency || ""}`}` : ""; const reasons = item.scoreReasons?.length ? `\nПочему: ${item.scoreReasons.slice(0, 3).join("; ")}` : ""; const url = item.sourceMessageUrl || item.url; return `${item.title || item.description || "Возможность"}${budget}${sources}${reasons}${url ? `\n${url}` : ""}`; }
...
export function createNotifications(items, chatId, thresholds = {}) {
  ...
  for (const item of items) {
    ...
    if (item.eligibility !== "LOW_PRIORITY" && Number(item.ageMinutes) < 60 && Number(item.proposalCost) === 0 && Number(item.skillMatch) >= 80 && Number(item.preliminaryScore) >= 75) notices.push({ opportunityId: item.id, kind: "fast", chatId, text: `⚡ СВЕЖИЙ ЗАКАЗ\nPreliminary analysis\n${details(item)}` });
